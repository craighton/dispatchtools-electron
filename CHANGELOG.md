...
- Hide the File/Edit/View/Window/Help menu bar on Windows and Linux — the
  toolbar chrome covers navigation. Menu keyboard shortcuts still work and Alt
  reveals the bar; macOS (system menu bar) is unchanged.
- Require Node.js >= 22.12.0 (Electron 42's minimum), declared in `package.json`
  `engines`, pinned in `.nvmrc`, and consumed by CI via `node-version-file`.
- Update Electron to 42.4.1 (patch).
